...
module tb_AES_ENCRYPT;
  ...
  // wait ciphertext_valid with timeout
  ...
  // ST_IDLE=0 ST_CORE=1 ST_WAITRK=2 
  task show_state;
    input [1:0] st;
    begin
      case (st)
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (start || busy || rk_valid || ciphertext_valid) begin
        $write("[%0t] ", $time);
        ...
        if (ciphertext_valid) begin
          $write(" | CT_VALID ct=%h", ciphertext);
        end
  ...
  initial begin
    ...
    if (ciphertext !== 128'h69c4e0d86a7b0430d8cdb78070b4c55a) begin
      ...
      $display("[AES-128] PASS: ct=%h", ciphertext);
    end

    wait_not_busy_timeout(500);
    ...
    if (ciphertext !== 128'hdda97ca4864cdfe06eaf70a0ec0d7191) begin
      ...
      $display("[AES-192] PASS: ct=%h", ciphertext);
    end

    wait_not_busy_timeout(800);
    repeat (2) @(posedge clk);
    ...
    if (ciphertext !== 128'h8ea2b7ca516745bfeafc49904b496089) begin
      ...
      $display("[AES-256] PASS: ct=%h", ciphertext);
    end

    wait_not_busy_timeout(1200);
